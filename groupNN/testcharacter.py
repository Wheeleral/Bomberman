    ...
    def _validate(self, x, y, gridCells, width, end, cost):  # check if wall
        ...
        if(x >= 0 and x < gridCells.width() and y >= 0 and y <gridCells.height()):
            if (not gridCells.wall_at(x,y)):  # convert from x,y to grid cell number
                ...
                if (cellCost != 0):
                    return Node(self, (x, y), self.g + cellCost, heuristic((x, y), end))  # creates new node using cost map
                else:
                    return Node(self, (x, y), self.g + 1, heuristic((x, y), end))  # create new node using default
            else:
           	    return None
        else:
            return None
...
def manhattan(p1, p2):  # use manhattan distance to get current distance
    return abs(p1[0] - p2[0]) + abs(p2[1] - p2[1])

def heuristic(p1, p2):  # use Euclidean distance to get dis to goal
    x = abs(p1[0] - p2[0])
    y = abs(p1[1] - p2[1])
    return sqrt((x * x) + (y * y))
