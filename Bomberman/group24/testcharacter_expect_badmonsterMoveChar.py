...
class TestCharacter(CharacterEntity):			
    ...
    def exp_value(self, wrld, a, depth):
        ...
        closest_monster = self.find_monster(wrld, a)
        #print("monster loc:", closest_monster)

        # if monster no longer exists
        if not closest_monster[0]:
            v = v + (self.max_value(wrld, a, depth + 1)) 
            return v
     
        # get successors of that monster in copied worlds
        monster_actions = self.get_monster_actions(wrld, closest_monster[0][1])
        ...
        for action in monster_actions: 
            ...
            #p = 1/8
            # pass copied world with new monster loc and the old action for the
            # character
    ...
    def monster_prob(self, wrld, char_loc, monster_loc):
        ...
        #print("char_loc:", char_loc, "monster_loc:", monster_loc, "weight:",
        #1/euclidean)

        # monster will probably chase character, therefore, higher weight goes
        # to actions
    ...
    def score_state(self, wrld, action):
        ...
        """"
        if self.nearMonsterV(x, y, wrld,1):
	        score += -10000
		
        if self.nearMonsterV(x, y, wrld, 2):
	        score += -1400
		
        if self.nearMonsterV(x, y, wrld, 3):
	        score += -1000

        if self.nearMonsterV(x, y, wrld,4):
	        score += -800
        if self.nearMonsterV(x, y, wrld,5):
	        score += -600

        if self.nearMonsterV(x, y, wrld,6):
	        score += -450
        """
        ...
        if  closest_monster[1]:
            #print("closests is " , closest_monster[1], "v" , (4 / closest_monster[1]) * -3000)
        ...
        if self.nearBomb(x, y, wrld) > 0:
            ...
            score += -(5 / self.nearBomb(x,y,wrld)) * 500
        ...
        if closest_monster[1]:
            if closest_monster[1] > 0:
                ...
                score -= ((0.65 * (100)) * (1 / closest_monster[1]))
            else: # close to us
        ...
        if bomb_distance > 0:
            ...
            score -= ((0.05 * (100)) * (1 / bomb_distance))
        ...
        if wrld.bomb_at(x,y) is not None and wrld.bomb_at(x,y).timer > 0:
            if wrld.bomb_at(x,y).timer > 2:   # Close to exploding
                score -= ((0.05 * (100)) * (1 / wrld.bomb_at(x,y).timer))
        ...
        if num_empty_cells > 0: #corners and sides
            if num_empty_cells < 5: # Enough spaces to move
                #score += 0.15 * ((1 - (1 / num_empty_cells)) * 100)
                score -= ((0.10 * (100)) * (1 / num_empty_cells))
            else:
                score -= (0.10 * (100))
        ...
        overall_score = (dist_to_goal - score) + 0.65 * score + 0.35 * dist_to_goal
    ...
    def find_monster(self, wrld ,loc): 
        count = 0
        monsters = []
        closest_m = None
        closest_dist = 0
        ...
        for m in monsters:
            ...
            if (closest_m == None):
                closest_m = m
                #print("x", a, "y", b, "monst", m[1])
                closest_dist = sqrt((a - m[1][0])*(a - m[1][0]) + (b - m[1][1])*(b - m[1][1]))
            else:
                dist  = sqrt((a - m[1][0])*(a - m[1][0]) + (b - m[1][1])*(b - m[1][1]))
                if dist < closest_dist:
                    closest_m = m
                    closest_dist = dist
        
        return (closest_m, closest_dist)
    ...
    def nearMonster(self, x, y, wrld):
        if(wrld.monsters_at(x + 1, y) or wrld.monsters_at(x - 1, y) or wrld.monsters_at(x, y + 1) or wrld.monsters_at(x, y - 1) or wrld.monsters_at(x + 1, y + 1) or wrld.monsters_at(x + 1, y - 1) or wrld.monsters_at(x - 1, y + 1) or wrld.monsters_at(x - 1, y - 1)):
	        return True
        return False

    """checks if location is within two squares of a monster"""
    def nearMonster2(self, x, y, wrld):
	    if(wrld.monsters_at(x + 2, y) or wrld.monsters_at(x - 2, y) or wrld.monsters_at(x, y + 2) or wrld.monsters_at(x, y - 2) or wrld.monsters_at(x + 2, y + 1) or wrld.monsters_at(x + 2, y + 2) or wrld.monsters_at(x + 2, y - 2) or wrld.monsters_at(x + 2, y - 1) or wrld.monsters_at(x - 2, y + 2) or wrld.monsters_at(x - 2, y - 2) or wrld.monsters_at(x - 2, y + 1) or wrld.monsters_at(x - 2, y - 1)):
		    return True
	    else:
		    return False
    ...
    def nearMonsterV(self, x, y, wrld, v):
        for xs in range(-v, v + 1, 1):
	        for ys in range(-v, v + 1 ,1):
		        if(self._withinBound(x + xs, y + ys, wrld)):
			        if(wrld.monsters_at(x + xs, y + ys)):
				        return True
        return False
